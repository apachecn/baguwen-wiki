...
Spring 3 中为 Bean 定义了 5 中作用域，分别为：
...
request：**在一次 Http 请求中，容器会返回该 Bean 的同一实例**。而对不同的 Http 请求则会产生新的 Bean，而且**该 bean 仅在当前 Http Request 内有效** Http 请求结束，该 bean实例也将会被销毁。
...
Spring 装配包括**手动装配和自动装配**，手动装配是有基于 xml 装配、构造方法、setter 方法等自动装配有五种自动装配的方式，可以用来指导 Spring 容器用自动装配方式来进行依赖注入。
